...
def animate(i):
    ...
    if(data != ''):
        ...
        plt.plot(x, sensor1, linestyle='solid', label ='Right Sensor 1', color = '#124C90')
        plt.plot(x, mva1, linestyle='dotted', label ='MVA Right Sensor 1', color="#124C90")

        plt.plot(x, sensor2, linestyle='solid', label ='Right Sensor 2', color = '#931C46')
        plt.plot(x, mva2, linestyle='dotted', label ='MVA Right Sensor 2', color="#931C46")

        plt.legend(loc='upper right')
        plt.xlabel('Time (s)')

        plt.ylabel('Distance (cm)')
        plt.ylim([0, max(max(sensor1), max(sensor2)) + 1])
